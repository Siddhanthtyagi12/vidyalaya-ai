...
def cleanup_user_files(user_id):
    """Removes user from encodings.pkl and names.txt."""
    try:
        encodings = load_encodings()
        # Convert user_id to string if needed, as pkl keys might be strings
        uid_str = str(user_id)
        if uid_str in encodings:
            del encodings[uid_str]
            save_encodings(encodings)
        elif user_id in encodings:
            del encodings[user_id]
            save_encodings(encodings)
            
        if uid_str in names_dict:
            del names_dict[uid_str]
            with open(os.path.join(basedir, "names.txt"), "w") as f:
                for uid, name in names_dict.items():
                    f.write(f"{uid}:{name}\n")
        elif user_id in names_dict:
            del names_dict[user_id]
            with open(os.path.join(basedir, "names.txt"), "w") as f:
                for uid, name in names_dict.items():
                    f.write(f"{uid}:{name}\n")
        return True
